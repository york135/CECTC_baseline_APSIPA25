...
def eval_all(answer_true, answer_pred, onset_tolerance=0.05
    , print_result=True, id_list=None, output_matched_notes=False
    , print_detail=False):
...
class MirEval():
    ...
    def prepare_data(self, gt_path, tr_path):
        ...
        for i in tr.keys():
            if i in gt.keys():
                gt[i] = [[float(gt[i][j][0]), float(gt[i][j][1]), int(gt[i][j][2])] for j in range(len(gt[i]))]
                gt_data.append(gt[i])
                tr_data.append(tr[i])
                id_list.append(i)
    ...
    def accuracy(self, onset_tolerance, print_result=True, output_matched_notes=False
        , print_detail=False):
        return eval_all(self.gt, self.tr, onset_tolerance=onset_tolerance, print_result=print_result
            , id_list=self.id_list, output_matched_notes=output_matched_notes
...
def main(args):
    ...
    print (time.time())
    my_eval.accuracy(onset_tolerance=float(args.tol), print_detail=args.print_detail)
    print (time.time())
