...
class CQT_feature_extractor():
    ...
    def compute_svs(self, y, sr, device):
        from demucs import pretrained, apply
        mix = torch.tensor([y, y]).float().to(device)
        ...
        output = sources[model.sources.index('vocals')].detach().cpu().numpy().T

        voc = (output[:,0] + output[:,1]) / 2
        acc = y - voc

        return voc, acc

    def get_feature_from_mixture(self, wav_path, do_svs, device):
        ...
        if do_svs == True:
            y_voc, y_acc = self.compute_svs(y, 44100, device=device)
