...
class NoteLevelAST:
    ...
    def fit(self, model_save_dir, **training_args):
        ...
        self.optimizer = optim.Adam(self.model.parameters(), lr=self.lr, betas=(0.9, 0.999), weight_decay=1e-3)
        ...
        for epoch in range(1, self.epoch + 1):
            ...
            if epoch % self.save_every_epoch == 0:
                ...
                with torch.no_grad():
                    ...
                    for batch_idx, batch in tqdm(enumerate(self.valid_loader)):

                        input_tensor = batch[0].to(self.device)

                        model_output = self.model(input_tensor)
                        loss = self.loss_function_class(batch, model_output, split_val_loss, use_ctc=False, use_ce=True)
                        ...
                        for i in range(len(onset_probs[bid])):
                            ...
                            if return_pitch_logit:
                                frame_info = (onset_probs[bid][i], offset_probs[bid][i], torch.argmax(pitch_octave_logits[bid][i]).item()
                                    , torch.argmax(pitch_class_logits[bid][i]).item(), result_tuple[4][bid][i].cpu().numpy())
                            else:
                                frame_info = (onset_probs[bid][i], offset_probs[bid][i], torch.argmax(pitch_octave_logits[bid][i]).item()
